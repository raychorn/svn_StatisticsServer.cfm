		...
		<tr>
			...
			<td width="50%" align="left" valign="top">
				...
				<cfif (serverNum eq 0)>
					...
					<cfset _totalNum = (aStruct.server1.numHits + aStruct.server2.numHits)>
				<cfelse>
					<cfset _serverIDText = " for Server (#serverNum#)">
					<cfset _totalNum = aStruct["server#serverNum#"].numHits>
				</cfif>
				...
				<cfif (IsDefined("cfHTTP1")) AND (IsDefined("cfHTTP1.Statuscode"))>
					...
					<span class="textYellowBoldClass"><NOBR>Total#_totalText# Hits#_serverIDText#: #_totalNum#</NOBR> <NOBR>ColdFusion <cfif (IsDefined("cfStruct")) AND (IsStruct(cfStruct))>#cfStruct.COLDFUSION.PRODUCTLEVEL# #cfStruct.COLDFUSION.PRODUCTVERSION#</NOBR> for <NOBR>#cfStruct.OS.NAME# #cfStruct.OS.ARCH#</NOBR></cfif> is </span>
