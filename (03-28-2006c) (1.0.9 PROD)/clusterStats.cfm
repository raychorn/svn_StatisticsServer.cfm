	...
	<table width="100%" cellpadding="-1" cellspacing="-1">
		...
		<cfif (UCASE(CGI.REQUEST_METHOD) eq "GET")>
			<cfscript>
				...
				safely_execSQL('qGetHistory', DSN, "SELECT serverNum, elapsedMs, beginDt FROM ClusterStats WHERE (jobStep = 'T') ORDER BY beginDt;");
			</cfscript>
		...
		<cfelseif (IsDefined("FORM.BEGINDT")) AND (IsDefined("FORM.ENDDT"))>
			<tr>
				<td align="left" valign="top">
					<cfscript>
						DSN = 'clusterDB';
	
						safely_execSQL('qGetHistory', DSN, "SELECT serverNum, elapsedMs FROM ClusterStats WHERE (jobStep = 'T') AND (beginDt >= CAST('00:00:00.0 #FORM.BEGINDT#' as datetime)) AND (beginDt <= CAST('23:59:59.9 #FORM.ENDDT#' as datetime))");
					</cfscript>
