...
<cfoutput>
	...
	<head>
		...
		<script language="JavaScript1.2" type="text/javascript">
			...
			function changeClusterMethod(sObj) {
				if ( (!!sObj) && (!!sObj.options) && (sObj.selectedIndex > -1) ) {
					document.location.href = '#CGI.SCRIPT_NAME#?clusterMethod=' + sObj.options[sObj.selectedIndex].value + '&nocache=' + uuid() + '&debugMode=#debugMode#';
				}
			}
		</script>
	</head>
	...
	<cfelseif (Len(clusterMethod) gt 0)>
		<cfscript>
			...
			safely_execSQL('qGetServer1Hits', DSN, "SELECT COUNT(id) as cnt FROM ClusterStats WHERE(serverNum = 1) AND (jobStep = 'T')");
			...
			safely_execSQL('qGetServer2Hits', DSN, "SELECT COUNT(id) as cnt FROM ClusterStats WHERE(serverNum = 2) AND (jobStep = 'T')");
